...
const LeaderboardComponent: FC<ILeaderboardComponent> = ({
  userData,
  dataList,
  isEmbedded = false,
}) => {
  const [sortKey, setSortKey] = useState<keyof LeaderboardData>('rank')
  ...
  const getSortSymbol = (key: keyof LeaderboardData) => {
    if (sortKey !== key) {
      return ''
    }
  ...
  const tableClasses = isEmbedded
    ...
    : 'mt-6 w-full border-collapse border border-gray-300'
  ...
  return (
    <div className={isEmbedded ? '' : 'flex flex-col items-center'}>
      ...
      <div className="space-y-2 mb-4 text-[#585656]">
        ...
        <div className="flex gap-4">
          ...
          <span>{formatTime(userData.time)}</span>
        </div>
      </div>
      ...
      <table className={tableClasses}>
        <thead>
          <tr>
            <th className={isEmbedded ? 'w-[5em] p-1' : 'w-[8em] p-1'}>
              <button
            ...
            <th className="p-1">
              <button
                ...
                time {getSortSymbol('time')}
              </button>
            </th>
          </tr>
        </thead>
        <tbody>
          {sortedDataList.map(({ rank, name, time }) => (
            <tr
              key={rank}
              className={`text-center ${
                rank === userData.rank ? 'text-[#0E7A11]' : ''
              }`}>
              <td className="p-1">{rank}</td>
              <td className="p-1">{name}</td>
              <td className="p-1">{formatTime(time)}</td>
            </tr>
          ))}
        </tbody>
      </table>
