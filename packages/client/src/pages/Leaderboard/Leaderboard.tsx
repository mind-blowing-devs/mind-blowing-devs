...
const leaderboardData = [
  { rank: 1, name: 'Player 1', time: '1:55' },
  { rank: 2, name: 'Player 2', time: '2:02' },
  { rank: 3, name: 'Player 3', time: '2:12' },
  { rank: 4, name: 'Player 4', time: '3:10' },
  { rank: 5, name: 'Player 5', time: '12:10' },
  { rank: 6, name: 'Player 6', time: '13:10' },
]
...
export default function Leaderboard() {
  return (
    <main className="font-press bg-[#BFBFBF] flex flex-col items-center justify-center min-h-screen p-4 sm:p-6">
      ...
      <div className="border-4 border-[#818181] bg-[#D9D9D9] p-6 mt-12 w-full max-w-xl text-xs sm:text-sm md:text-base">
        ...
        <ol className="mt-6 list-decimal list-inside">
          {leaderboardData.map((player, index) => (
            <li
              key={index}
              className={`flex justify-between font-medium py-2 ${
                player.rank === playerData.rank
                  ? 'text-[#0E7A11] font-bold'
                  : ''
              }`}>
              <span className="flex gap-2">
                ...
                <span
                  className={
                    player.rank !== playerData.rank ? 'text-[#585656]' : ''
                  }>
                  {player.name}
                </span>
              </span>
      ...
      <Link
        to="/game"
        className="my-8 hover:text-gray-500 text-sm"
