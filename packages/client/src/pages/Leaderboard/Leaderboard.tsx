...
export default function Leaderboard() {
  const [dataList] = useState<MockData[]>(mockDataList)
  const [sortKey, setSortKey] = useState<keyof MockData>('rank')
  ...
  const handleSort = (key: keyof MockData) => {
    setSortOrder(prev => (sortKey === key && prev === 'asc' ? 'desc' : 'asc'))
    setSortKey(key)
  }

  const getSortSymbol = (key: keyof MockData) => {
    if (sortKey !== key) {
      return ''
    }
    ...
    <main className="font-press bg-[#BFBFBF] flex flex-col items-center justify-center min-h-screen p-4 sm:p-6">
      ...
      <div className="border-4 border-[#818181] bg-[#D9D9D9] p-4 sm:p-6 mt-12 w-full max-w-xl text-xs sm:text-sm md:text-base">
        ...
        <div className="flex gap-4">
          <span>your rank:</span>
          <span>{mockUserData.rank}</span>
        </div>
        <div className="flex gap-4">
          <span>your best time:</span>
          <span>{formatTime(mockUserData.time)}</span>
        </div>
        <table className="mt-6 w-full border-collapse border border-gray-300">
          <thead>
            <tr>
              <th className="w-[5em] sm:w-[8em] p-1">
                <button
          ...
          <tbody>
            {sortedDataList.map(({ rank, name, time }) => (
              <tr
                key={rank}
                className={`text-center ${
                  rank === mockUserData.rank ? 'text-[#0E7A11]' : ''
                }`}>
