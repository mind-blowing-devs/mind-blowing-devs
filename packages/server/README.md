...
3. [Ручки API](#3-ручки-api)
   ...
   - [Reaction API](#34-reaction-api)
   - [VisualTheme API](#35-visualTheme-api)
4. [Ошибки](#4-ошибки)
5. [Модели сущностей](#5-модели-сущностей)
   - [Topic](#51-topic)
   - [Comment](#52-comment)
   - [Reply](#53-reply)
   - [Reaction](#54-reaction)
   - [VisualTheme](#55-visualtheme)
   - [UserVisualTheme](#56-uservisualtheme)
   - [User](#57-user)
6. [Проверка Авторизации](#6-проверка-авторизации)
7. [Добавление данных на старте](#7-добавление-данных-на-старте)
...
#### 3.4.1. Получить список реакций для ответа
...
Возвращает список эмодзи с количеством реакций и статусом реакции текущего пользователя:
...
#### 3.5.2. Получить список тем оформления
...
#### 3.5.4. Установить тему оформления для пользователя
...
#### 3.5.5. Получить тему оформления пользователя
...
- **replyId** (UUID): ID ответа, к которому относится реакция;
- **userId** (number): ID пользователя, который добавил реакцию;
- **emoji** (string): Символ эмодзи;
- **createdAt** (Date): Дата создания реакции;
- **updatedAt** (Date): Дата последнего обновления.
...
### 5.6. UserVisualTheme. Таблица для свзки пользователя и темы оформления
